...
const generatePluginCss = (config) => postcss(
	tailwindcss(
		merge({
			theme: {},
			corePlugins: false,
...
test("uses key but no extra config", () => generatePluginCss({
	theme: {
		mixBlendMode: {
			screen: "screen",
			overlay: "overlay",
...
}).then((css) => expect(css).toMatchCss(`
...
test("variants (hover and focus) with key and no extra config", () => generatePluginCss({
	theme: {
		mixBlendMode: {
			screen: "screen",
			overlay: "overlay",
...
test("rename with responsive variants", () => generatePluginCss({
	theme: {
		extend: {
			customUtilities: {
				filter: { rename: "" },
...
test("property with active variants", () => generatePluginCss({
	theme: {
		extend: {
			blur: {
				0: "blur(0)",
...
test("multiple custom utilities with rename", () => generatePluginCss({
	theme: {
		customUtilities: {
...
test("can use other plugins (tailwindcss-pseudo)'s variants", () => generatePluginCss({
	theme: {
		extend: {
			content: {
				empty: "''",
...
test("negative properties are prefixed with - without variants", () => generatePluginCss({
	theme: {
		extend: {
			customUtilities: {
...
test("negative properties are prefixed with - when renamed without variants", () => generatePluginCss({
	theme: {
		margin: {
			"-4": "-1rem",
			"-2": "-0.5rem",
...
test("negative properties are prefixed with - with variants", () => generatePluginCss({
	theme: {
		extend: {
			customUtilities: {
...
test("negative properties are prefixed with - when renamed with variants", () => generatePluginCss({
	theme: {
		tracking: {
			"-3": "-3px",
			"-2": "-2px",
